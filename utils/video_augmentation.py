...
class RandomRotation(object):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            # FIXED: Replaced scipy with cv2 rotation
            rotated = []
            for img in clip:
                h, w = img.shape[:2]
...
class RandomResize(object):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            # FIXED: Replaced scipy with cv2 resize
            return [cv2.resize(img, (new_w, new_h)) for img in clip]
        elif isinstance(clip[0], PIL.Image.Image):
    ...
    def _get_PIL_interp(self, interp):
        if interp == 'nearest': return PIL.Image.NEAREST
        elif interp == 'lanczos': return PIL.Image.LANCZOS
        elif interp == 'bilinear': return PIL.Image.BILINEAR
        elif interp == 'bicubic': return PIL.Image.BICUBIC
        elif interp == 'cubic': return PIL.Image.CUBIC

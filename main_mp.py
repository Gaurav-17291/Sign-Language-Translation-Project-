...
def train_model(loader, model, optimizer, local_rank, epoch_idx, recoder):
    ...
    for batch_idx, data in enumerate(pbar):
        vid_full = data[0].cuda(local_rank)
        vid_face = data[1].cuda(local_rank)
        vid_lh = data[2].cuda(local_rank)
        vid_rh = data[3].cuda(local_rank)

        vid_lgt = data[4].cuda(local_rank)
        label = data[5].cuda(local_rank)
        label_lgt = data[6].cuda(local_rank)
        ann = data[7]

        optimizer.optimizer.zero_grad(set_to_none=True)

        with torch.cuda.amp.autocast():
            ret_dict = model(vid_full, vid_face, vid_lh, vid_rh, vid_lgt, label=label, label_lgt=label_lgt, ann=ann)
...
def eval_model(cfg, loader, model, local_rank, mode, epoch, work_dir, recoder, evaluate_tool="python"):
    ...
    for batch_idx, data in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            ret_dict = eval_model(vid_full, vid_face, vid_lh, vid_rh, vid_lgt, label=label, label_lgt=label_lgt)
    ...
    try:
        ...
        if local_rank == 0: print("Unexpected error in evaluation:", e)
        lstm_ret = 100.0
